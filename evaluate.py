...
parser.add_argument('--model', type=str, default='model_/model_44.pth', metavar='M',
    ...
    with torch.no_grad():
        for i in range(0,len(transforms)):
            ...
            for img in img_paths:
                data = transforms[i](pil_loader(img))
                data = data.view(1, data.size(0), data.size(1), data.size(2))    # 1, 3, 32, 32
                data = Variable(data)
